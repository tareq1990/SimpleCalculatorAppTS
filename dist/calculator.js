...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function isWithinOperandRange(operand) {
    ...
    function disableEqualsButton(disabled) {
    ...
    display.addEventListener('input', function (event) {
        const input = event.target; // Get the input box
        const regex = /^[0-9+\-*/.]*$/; // Allowed characters: digits, operators, and decimal points
        ...
        if (len > 1) {
            ...
            const lastSecondIsOperator = OPERATORS.includes(input.value[len - 2]); // Check if the second-to-last character is also an operator
